...
$result2 = $total_daily_calorie_need*0.85;$result22 = number_format($result2,2);
$result3 = $total_daily_calorie_need*0.8; $result33 = number_format($result3,2);
$result4 = $total_daily_calorie_need*0.75; $result44 = number_format($result4,2);
$result5 = $total_daily_calorie_need*0.7; $result55 = number_format($result5,2);
if($result> $limit || $result1> $limit || $result2> $limit || $result3> $limit || $result4> $limit || $result5> $limit){
echo"<p class='panel-body' style='padding:0px 15px;margin:0px;'><strong>To lose weight, choose an option from the table below.";if($type=='metric'){echo" One pound of weight is lost for every 3000";}else{echo"One Kilo of weight is lost for every 3500";} echo" calories reduced or burned.</strong><br><br></p><h3 class='panel-heading'><strong>Weight Lose Table</strong></h3>";
echo"<table class='table table-striped'><thead class='panel-heading'><tr class='danger'><td>Calorie Reduction from BMR</td><td>Calories per day</td></tr></thead><tbody>";
if($type=='metric'){$message1='1 Kg/week';$message2='2 Kg/week';}else{$message1='1 lb/week';$message2='2 lb/week';}
if($result > $limit){echo "<tr><td>500 Calories Less (approx $message1)</td><td>" . $resultt . "</td></tr>";}
if($result1 > $limit){echo "<tr><td>1000 Calories (approx $message2)</td><td>" . $result11 . "</td></tr>";}
if($result2 > $limit){echo "<tr><td>15% less</td><td>" . $result22 . "</td></tr>";}
if($result3 > $limit){echo "<tr><td>20% less</td><td>" . $result33 . "</td></tr>";}
if($result4 > $limit){echo "<tr><td>25% less</td><td>" . $result44 . "</td></tr>";}
if($result5 > $limit){echo "<tr><td>30% less</td><td>" . $result55 . "</td></tr>";}
echo"</tbody></table>";
}
echo"</div>";
}
